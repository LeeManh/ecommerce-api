...
public class JwtService {
    ...
    private SecretKey accessTokenKey() {
            return Keys.hmacShaKeyFor(jwtProperties.accessToken().secret().getBytes(StandardCharsets.UTF_8));
    }

    private SecretKey refreshTokenKey() {
        return Keys.hmacShaKeyFor(jwtProperties.refreshToken().secret().getBytes(StandardCharsets.UTF_8));
    }
}
